...
Version allégée sans dépendances complexes
"""
...
# Classe simple pour lire les FCS
...
# Fonctions de gating simplifiées
...
def gate_positive_simple(data, channel, method='kmeans'):
    ...
    # Identifier le cluster "positif" (plus haute moyenne)
    positive_cluster = np.argmax(kmeans.cluster_centers_)
    
    # Prédire pour toutes les données
...
with st.sidebar:
    ...
    mode = st.radio(
        "Mode d'analyse",
        ["🔍 Analyse Simple", "📊 Informations du Fichier"],
        index=0
    )
    ...
    **Fonctionnalités** :
    - ✅ Lecture fichiers FCS
    - ✅ Gating basique
    - ✅ Visualisations
    - ✅ Export Excel
    
    **Note** : Version simplifiée pour Streamlit Cloud
    
    Pour la version complète, utiliser en local avec le package complet.
    """)
...
if mode == "🔍 Analyse Simple":
    ...
    uploaded_file = st.file_uploader(
        "Sélectionner un fichier FCS",
        type=['fcs'],
        help="Format FCS 2.0, 3.0 ou 3.1"
    )
    ...
    if uploaded_file is not None:
        ...
        st.markdown(f'<div class="info-box">📄 Fichier : <b>{uploaded_file.name}</b></div>', 
                   unsafe_allow_html=True)
        ...
        try:
            ...
            if st.button("🚀 Analyser", type="primary", use_container_width=True):
                with st.spinner("Analyse en cours..."):
                    ...
                    marker_channels = [c for c in data.columns 
                                     if any(m in c.upper() for m in ['CD', 'FITC', 'PE', 'APC'])]
                    
                    if marker_channels:
                        st.info(f"🔍 {len(marker_channels)} marqueurs détectés")
                        
                        # Gating sur premier marqueur
                        if marker_channels:
                            parent = gates.get('viable', gates.get('singlets', 
                    ...
                    st.markdown('<div class="success-box">✅ Analyse terminée !</div>', 
                               unsafe_allow_html=True)
            ...
            if st.session_state.analysis_done and st.session_state.gates:
                ...
                with tab2:
                    ...
                    with col1:
                        x_channel = st.selectbox("Canal X", all_channels, 
                                                index=0 if all_channels else None)
                    with col2:
                        y_channel = st.selectbox("Canal Y", all_channels,
                                                index=1 if len(all_channels) > 1 else 0)
                    ...
                    if x_channel and y_channel:
                        ...
                        # Background
                        ax.scatter(reader.data[x_channel], reader.data[y_channel],
                        ...
                        for idx, (gate_name, mask) in enumerate(st.session_state.gates.items()):
                            ...
                            ax.scatter(reader.data.loc[mask, x_channel],
                                     reader.data.loc[mask, y_channel],
                                     s=2, c=color, alpha=0.5, 
                                     label=f"{gate_name} ({mask.sum():,})",
                                     rasterized=True)
                ...
                with tab3:
                    ...
                    st.markdown("---")
                    st.info("💡 Pour export Excel complet, utiliser la version locale")
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            for category, marker_list in markers.items():
                found = [ch for ch in info['channels'] 
                        if any(m.upper() in ch.upper() for m in marker_list)]
                if found:
...
<div style='text-align: center; color: gray; padding: 1rem;'>
    ...
    <p>Version simplifiée pour Streamlit Cloud | Pour fonctionnalités complètes : utiliser localement</p>
</div>
""", unsafe_allow_html=True)
