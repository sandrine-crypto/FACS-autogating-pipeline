#!/usr/bin/env python3
"""
FACS Autogating - Gates Hexagonaux v4
- Affichage correct des événements
- Gates hexagonaux modifiables
...
def auto_gate_hexagon(data, x_ch, y_ch, parent_mask=None, mode='main'):
    """Auto-gating retournant un hexagone"""
    ...
    try:
        ...
        # Sélection cluster
        if mode == 'main':
            target = np.argmax([np.sum(labels == i) for i in range(2)])
        elif mode == 'low_x':
            target = np.argmin([np.mean(xt[labels == i]) for i in range(2)])
        elif mode == 'high_x':
            target = np.argmax([np.mean(xt[labels == i]) for i in range(2)])
        else:
            target = 0
        
        cluster_mask = labels == target
        cx, cy = xt[cluster_mask], yt[cluster_mask]
        ...
        return create_hexagon(center_x, center_y, max(radius_x, 10), max(radius_y, 10))
...
def create_plot(data, x_ch, y_ch, x_label, y_label, title, polygon, parent_mask, gate_name):
    ...
    # Scatter plot - IMPORTANT: utiliser Scatter standard, pas Scattergl
...
st.markdown('<h1 class="main-header">🔬 FACS - Hexagones Interactifs v4</h1>', unsafe_allow_html=True)
...
if uploaded:
    if st.session_state.reader is None or st.session_state.get('fname') != uploaded.name:
        ...
        with st.spinner("Chargement..."):
            ...
            st.session_state.channels = {
                ...
                'CD3': find_channel(cols, ['AF488', 'FITC']),
    ...
    # Debug: afficher les canaux trouvés
        ...
        for row in range(2):
            ...
            for col_idx in range(2):
                ...
                with cols_display[col_idx]:
                    ...
                    if poly:
                        with st.expander(f"✏️ Modifier {gname}"):
                            # Point par point
                            col1, col2 = st.columns(2)
                            with col1:
                                pt_idx = st.selectbox("Point", range(6), format_func=lambda x: f"Pt {x+1}", key=f"pt_{gkey}")
                            with col2:
                                st.text(f"Actuel: ({poly[pt_idx][0]:.1f}, {poly[pt_idx][1]:.1f})")
                            
                            col_x, col_y = st.columns(2)
                            with col_x:
                                new_x = st.number_input("X", value=float(poly[pt_idx][0]), step=5.0, key=f"x_{gkey}")
                            with col_y:
                                new_y = st.number_input("Y", value=float(poly[pt_idx][1]), step=5.0, key=f"y_{gkey}")
                            
                            if st.button("✅ Appliquer", key=f"apply_{gkey}"):
                                new_poly = list(poly)
                            ...
                            step = st.slider("Pas", 5, 50, 15, key=f"step_{gkey}")
                            ...
                            c5, c6 = st.columns(2)
                            if c5.button("➕ Agrandir", key=f"grow_{gkey}"):
                                st.session_state.polygons[gkey] = scale_polygon(poly, 1.15)
                                st.rerun()
                            if c6.button("➖ Réduire", key=f"shrink_{gkey}"):
    ...
    st.markdown("""
    <div class="info-box">
    <h3>🔬 FACS Auto-Gating - Hexagones v4</h3>
    <ul>
    <li>Gates <b>hexagonaux</b> (6 sommets)</li>
    <li>Auto-gating <b>GMM</b></li>
    <li>Modification <b>point par point</b></li>
    <li>Mise à jour <b>cascade</b></li>
    </ul>
    <p>Uploadez un fichier FCS</p>
    </div>
    """, unsafe_allow_html=True)

st.caption(f"v4 | 🧠 {n_learned} corrections")
